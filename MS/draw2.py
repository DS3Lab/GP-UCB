...
with_prior=[]
for line in open("ucb_accuracy1.txt",'r'):
	with_prior.append(line)
without_prior=[]
for line in open("ucb_accuracy2.txt",'r'):
	without_prior.append(line)

ei_with_prior=[]
for line in open("ei_accuracy1.txt",'r'):
	ei_with_prior.append(line)
ei_without_prior=[]
for line in open("ei_accuracy2.txt",'r'):
	ei_without_prior.append(line)

latest=[]
for line in open("latest_first_without.txt",'r'):
	latest.append(line)
most_cited=[]
for line in open("cited_first_without.txt",'r'):
	most_cited.append(line)
test=[]
for line in open("test_without_googlenet.txt",'r'):
	test.append(line)
...
for i in range (9):
	...
	max3 = 0
	max4 = 0
	a = random.sample([0,1,2,3,4,5,6,7],8)
	for aa in a:
...
#a5_2 = [0.166   ,  0.29 , 0.53]
	plt.plot(size, a_with_prior, c = 'g', marker='x', label = "GP-UCB")
	#plt.plot(size, a_without_prior, 'r',marker='x', label = "GP-UCB (without prior)")
	plt.plot(size, a_ei_with_prior, 'y--',marker='.', label = "EI")
	...
	if i == 0:
		plt.axis([0,7,0.52,0.78])
	elif i == 1:
		plt.axis([0,7,0.02,0.06])
	elif i == 2:
		plt.axis([0,7,0.03,0.15])
	elif i == 3:
		plt.axis([0,7,0.18,0.32])
	elif i == 4:
		plt.axis([0,7,0.25,0.38])
	elif i == 5:
		plt.axis([0,7,0.38,0.57])
	elif i == 6:
		plt.axis([0,7,0.45,0.68])
	elif i == 7:
		plt.axis([0,7,0.55,0.74])
	elif i == 8:
		plt.axis([0,7,0.5,0.67])

	
	
	if i == 0:
		plt.title("Model selection (accuracy)[Imagenet]")
		plt.savefig("/users/serena/desktop/without/ImageNet")
	elif i == 1:
		plt.title("Model selection (accuracy)[Sculpture6k]")
		plt.savefig("/users/serena/desktop/without/Sculpture6k")
	elif i == 2:
		plt.title("Model selection (accuracy)[CUB-200]")
		plt.savefig("/users/serena/desktop/without/CUB-200")
	elif i == 3:
		plt.title("Model selection (accuracy)[MIT-67]")
		plt.savefig("/users/serena/desktop/without/MIT-67")
	elif i == 4:
		plt.title("Model selection (accuracy)[voc2012]")
		plt.savefig("/users/serena/desktop/without/VOC2012")
	elif i == 5:
		plt.title("Model selection (accuracy)[Caltech101]")
		plt.savefig("/users/serena/desktop/without/Caltech-101")
	elif i == 6:
		plt.title("Model selection (accuracy)[Oxford5k]")
		plt.savefig("/users/serena/desktop/without/Oxford5k")
	elif i == 7:
		plt.title("Model selection (accuracy)[Oxford-102]")
		plt.savefig("/users/serena/desktop/without/Oxford-102")
	elif i == 8:
		plt.title("Model selection (accuracy)[Cifar100]")
		plt.savefig("/users/serena/desktop/without/Cifar100")
