...
void init()
{
	...
	for (int i = 0; i < ALGO; i++)
	{
		...
		// Initially, \Sigma_0^2 is just diag{kernel matrix}
...
{
	my_cin(argv[1]); // prior accuracy
	calculate_prior_gaussian();
	cin_test(argv[2]); // test
	cin_cost(argv[3]); // prior cost
	init();
	...
	ofstream fout("predict_reduced.txt");

	int t = 1;
	for (int observe = 0; observe < TEST_DATASET; observe++)
	{
			t = 1;
			init(); // For every testing dataset, init the whole environment first
			...
			while (t <= max_iteration) // 
			{
				...
				while (1)
				{
					for (int i = 0; i < ALGO; i++)
					{
						...
						double tmp_reward = current_mean[i] + sqrt(beta[t] * 1.0/cost[i]) * sqrt(current_var[i]);
