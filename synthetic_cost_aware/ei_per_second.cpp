			...
			while (t <= max_iteration) // 
			{
				...
				while (1)
				{
					...
					for (int i = 0; i < ALGO; i++)
					{
						...
						double tmp_reward = delta * cdf(my_normal,delta * 1.0 / sqrt(current_var[i])) + 
						sqrt(current_var[i]) * pdf(my_normal, delta * 1.0/sqrt(current_var[i]));

						tmp_reward = tmp_reward * 1.0 / cost[i];
					...
					a[t] = algo;
					if (chosen[algo] > 1 && chosen[algo] < 3)
					{
						max_iteration = max_iteration+1;
						break;
					}
					else if (chosen[algo] >= 3)
					{
