...
#define TEST_DATASET 8
#define ITERATION 9
#define USER 8           // how many users
...
void calculate_prior_gaussian(int a)
{
	...
	for (int i = 0; i < ALGO; i++)
	{
		...
		{
			if (j % USER == a || j % USER == a + 1 || j % USER == a + 2 || j % USER == a + 3)
			{
	...
	for (int i = 0; i < ALGO; i++)
	{
		for (int j = 0; j < ALGO; j++)
		{
			...
			{
				if (k % USER == a || k % USER == a + 1 || k % USER == a + 2 || k % USER == a + 3)
				{
	...
	while (t <= ITERATION) // 
	{
		...
		for (int j = 0; j  < USER; j++)
		{
			...
			for (int i = 0; i < ALGO; i++)
			{
				...
				if (tmp_reward > max_reward && has_chosen[j][i] == 0)
				{
		...
		if (t == 1)
		{
			...
			for (int i = 0; i < USER; i++)
				if (sqrt(current_var[i][a[i][t]]) > max_sigma)
				{					
					max_sigma = sqrt(current_var[i][a[i][t]]);
					user_id = i;
				}
		}
		else
		{
			...
			for (int i = 0; i < USER; i++)
			{
				cout << "user " << i << " 's current var is: " <<	current_var[i][a[i][t]] << "  , and previous var is: " 
				<< pre_var[i][a[i][t-1]] << endl;
				if ((sqrt(current_var[i][a[i][t]]) -  sqrt(pre_var[i][a[i][t-1]]))> max_sigma)
				{	
					max_sigma = sqrt(current_var[i][a[i][t]]) - sqrt(pre_var[i][a[i][t-1]]);
					user_id = i;
				}
			}
		}
