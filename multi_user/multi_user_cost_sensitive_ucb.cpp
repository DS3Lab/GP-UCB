...
int has_chosen[USER+5]; // this user has chosen how many different algorithms
double cost[ALGO+5];
...
int main(int argc, char const *argv[])
{
	...
	while (1) // 
	{
		...
		for (int j = 0; j  < USER; j++)
		{
			...
			cout << algo+1 <<" has been chosen " << chosen[j][algo] << "times by user " << j << endl;
			a[j][t] = algo; // we use the actuall one to update		

			if (chosen[j][algo] > 1 && chosen[j][algo] < 5)
			{
				max_iteration = max_iteration+1;
			}
			else if (chosen[j][algo] >= 5)
			{
				...
				for (int i = 0; i < ALGO; i++)
				{
					...
					if (tmp_reward > max_reward && chosen[j][i] == 0)
					{
						max_reward = tmp_reward;
						algo = i;
					}
