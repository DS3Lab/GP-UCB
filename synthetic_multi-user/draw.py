...
number = 0
for line in open("2rr_2031.txt",'r'):
	line = line.strip("\n").split(" ")
...
plt.plot(x_time, y_accu, 'b',marker='.', label = "2-RR-order1")
...
for line in open("multi_user_most_variance_drop.txt",'r'):
	line = line.strip("\n").split(" ")
	sum_time = sum_time + float(time_[int(line[0])][int(line[1])])
	x_time.append(sum_time)
...
plt.plot(x_time, y_accu, 'r',marker='x', label = "Most Variance drop")
...
plt.plot(x_time, y_accu, 'y--',marker='x', label = "RR+Variance drop")


ax1.legend(loc=4)
plt.axis([-10,400,0.00,4.0])
plt.title("Multi-user cost-aware model selection")
plt.savefig("/users/serena/desktop/multi-user2")
