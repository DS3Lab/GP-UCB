...
#define TEST_DATASET 5
#define ITERATION 100
#define USER 5           // how many users
...
double sigma_t_k[USER+5][600];
double sigma_t[USER+5][600][600];
int a[USER+5][600]; // is the actual events that happen in the process
double C_t[USER+5][600][600]; // 10000 is time t;
...
double cost[USER+5][ALGO+5];
int actual_run[1000]; // the user_id which actually runs in each time stamp t
...
void calculate_prior_gaussian(int a)
{
	...
	for (int i = 0; i < ALGO; i++)
	{
		for (int j = 0; j < ALGO; j++)
		{
			...
			// use the default squared exponential kernel matrix
			//kernel[i][j] = 1;
			cout << kernel[a][i][j] << " ";
		}
		cout << endl;
...
void warm_start()
{
	...
	for (int j = 0; j  < USER; j++)
	{
		...
		for (int i = 0; i < ALGO; i++)
		{
			...
			double tmp_reward = current_mean[j][i] + sqrt(beta * 1.0/cost[j][i]) * sqrt(current_var[j][i]);
...
int main(int argc, char const *argv[])
{
	...
	ofstream fout("multi_user_rr+variance_drop_makesure.txt");
	...
	while (1) // 
	{
		...
		for (int j = 0; j  < USER; j++)
		{
			...
			for (int i = 0; i < ALGO; i++)
			{
				...
				//if (j == 0 || j == 1) cost[i] = 1;
				//else cost[i] = 8;
				double tmp_reward = current_mean[j][i] + sqrt(beta[t]*1.0/cost[j][i]) * sqrt(current_var[j][i]);
				//double tmp_reward = current_mean[j][i] + sqrt(beta[t]*1.0) * sqrt(current_var[j][i]);
				cout << "tmp_reward is: " << tmp_reward << endl;
				// In order to get it to work, I change the algorithm to: if this has been chosen by the user, then just 
				// pick up the second best algorithm
		...
		else
		{
			...
			for (int i = 0; i < USER; i++)
			{
				...
				// only consider the users that didn't run in the last round
